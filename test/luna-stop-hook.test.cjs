...
function fixture(status, pid = process.pid) {
  mkdirSync(directory, { recursive: true, mode: 0o700 });
  const id = `test_hook_${process.pid}_${Math.random().toString(16).slice(2)}`;
  const path = join(directory, `${id}.json`);
  writeFileSync(path, JSON.stringify({ threadId: id, pid, status, outputDir: "/tmp/out" }));
  return { id, path };
}
...
test("continues only the parent and wakes it after terminal or crash", () => {
  ...
  const crashed = fixture("active", 999_999_999);
